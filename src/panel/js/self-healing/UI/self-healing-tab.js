...
import { getCommandTargetOptions, isSelfHealingTabDisplay } from "../service/utils.js";
...
const addBrokenLocatorToSelfHealingTab = (brokenLocatorTestStep) => {
  ...
  let resultRow = $(`<tr>
                        <td>
                            ${brokenLocatorTestStep.testCase.innerText}
                        </td>
                        <td>Executing...</td>
  ...
  $(row).click(async function (e) {
    ...
    if (getSelectedCase().id === testCaseID) {
      ...
      for (let element of records) {
        ...
        let optionList = await getCommandTargetOptions(element, false);
