...
neighborXpathsGenerator.getXpathsByNeighbors = function (clickedElement, xpathPrefix) {
    ...
    for (var i = 0; i < usefulNeighbors.length; i++) {
        ...
        if (!neighborXpathsGenerator.isDescendant(thisUsefulNeighbor, clickedElement)) {
            ...
            partialPathToGetTagOffset = neighborXpath + '/' + correctPrefix + '::' + neighborXpathsGenerator.getElementTagNameOrSvgName(clickedElement);
            ...
            partialPathToGetTagOffset = neighborXpath + '//' + neighborXpathsGenerator.getElementTagNameOrSvgName(clickedElement);
            correctOffsetByTagName = neighborXpathsGenerator.getCorrectOffset(clickedElement, partialPathToGetTagOffset);
            newXPath = partialPathToGetTagOffset + "[" + correctOffsetByTagName + "]";
        }
        if(xpathPrefix){
    ...
    if(xpathPrefix) {
        newXpaths.push("xpath=" + neighborXpathsGenerator.generateTextLocator(clickedElement));
    } else {
        newXpaths.push(neighborXpathsGenerator.generateTextLocator(clickedElement));
    }
    return newXpaths;
}

neighborXpathsGenerator.generateTextLocator = function (element) {
    var current = element;
    ...
    return "//*/text()[normalize-space(.)='" + firstText.trim() + "']/parent::*";
}

/**
 * SVG is treated as a special case because it cannot be used as a tag name (makes no sense but whatever)
 */
neighborXpathsGenerator.getElementTagNameOrSvgName = function(element) {
